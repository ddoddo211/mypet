...
<head>
<meta charset="UTF-8">
<title>companion.jsp(보험금 신청현황 - 반려)</title>
...
<form action="/isr/gohistory2" method="get" id="frm">
		...
		<div id="isrApplyMain">
			...
			<div id="isrApplyTitle3">
				<div id="isrApplyTitle3_1">
					보험금 청구가 "반려"상태인 내역은 내역을 삭제하실수 없습니다.
				</div>
			</div>
			
			<div id="isrApplyTitle3_1_1">
				<div id="isrApplyTitle3_1_2">
					보험금 청구 상태가 "반려"상태인 청구건은 [보험금내역]을 확인하실수 있습니다.
				</div>
			</div>
			...
			<div id="companionTable">
						<table id="isrApplyTable1">
								<tr class="tr2">
									<th class="td2">선택</th>
									<th class="td5">보험금<br>청구대상</th>
									<th class="td5">보험상품</th>
									<th class="td1">사고일자</th>
									<th class="td5">사고장소</th>
									<th class="td5">상태</th>
									<th class="td1">반려사유</th>
								</tr>
...
<c:choose>	
	...
	<c:otherwise>		
							<c:forEach items="${companion}" var="list">
								<tr>
									<td class="td1"><input class="isrApplyChk" type="radio" name="isrApplyChk" value="${list.accd_id }"/></td>
									<td class="td5">${list.myp_name}</td>
									<td class="td5">${list.insp_name}</td>
									<td class="td1"><fmt:formatDate value="${list.accd_date}" pattern="yyyy년 MM월 dd일"></fmt:formatDate></td>
									<td class="td5">${list.accd_addr}</td>
									<td class="td5">${list.accd_stat}</td>
									<td class="td1">${list.accd_res}</td>
								</tr>
							</c:forEach>
	</c:otherwise>	
</c:choose>
